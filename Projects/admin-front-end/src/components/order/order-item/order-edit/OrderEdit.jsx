...
class OrderEdit extends Component {
    ...
    getOrderInformation = () => {
        fetch(`https://localhost:44376/api/admin/order/getOrderById?id=${this.props.orderId}`)
        .then(res => res.json())
        .then(res => {
            this.setState({
    ...
    renderOrderDetails = () => {
        if(this.state.orderDetails.length !== 0) {
            const details = this.state.orderDetails.map((order, idx) => 
                <OrderEditItem key={idx} index={idx} colorId={order.colorId} productId={order.productId} sizeId={order.sizeId} quantity={order.quantity} price={order.price}></OrderEditItem>
            );
    ...
    dateValidation = () => {
        ...
        window.$('#dtNewExpectationDelivery' + this.props.orderId).attr('min', today);
    }
    ...
    confirm = () => {
        fetch('https://localhost:44376/api/admin/order/editDeliveryDate', {
            ...
            body: JSON.stringify({
                ...
                deliveryDate: window.$('#dtNewExpectationDelivery' + this.props.orderId).val()
            })
        })
        .then(res => res.json())
        .then(res => {
    ...
    render() {
        ...
        <span className="input-group-text" id="basic-addon1">Ngày giao hàng dự kiến {this.state.orderInformation.deliveryDate}</span>
                            ...
                            <div className="input-group mb-3">
                                <div className="input-group-prepend">
                                    <span className="input-group-text" id="basic-addon1">Ngày giao hàng dự kiến mới</span>
                                </div>
                                <input type="date" id={"dtNewExpectationDelivery" + this.props.orderId} className="form-control" placeholder="Username" aria-label="Username" aria-describedby="basic-addon1" ref="deliveryDate"/>
                            </div>
                        </div>                 
                    </div>
                    <div className="modal-footer">
                        <button type="button" className="btn btn-danger" onClick={this.confirm}> Xác nhận </button>
