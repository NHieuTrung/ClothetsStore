    ...
    renderOrderDetails = () => {
        if(this.state.orderDetails.length !== 0) {
            const details = this.state.orderDetails.map((order, idx) => 
                <OrderDeliveryItem key={idx} index={idx} colorId={order.colorId} productId={order.productId} sizeId={order.sizeId} quantity={order.quantity} price={order.price}></OrderDeliveryItem>
            );
        ...
        .then(res => {
            if(res === true) {
                MySwal.fire({
                    ...
                    padding: '2em',
                    icon: 'success',
                    html: "<p style='font-size: 15px'>Xác nhận ngày giao hàng thành công</p>"
                }).then((res) => {
                        ...
                        <div className="container">
                            <div className="input-group mb-3">
                                <div className="input-group-prepend">
                                    <span className="input-group-text" id="basic-addon1">Ngày giao hàng dự kiến</span>
                                </div>
                                <input type="date" id={"dtDelivery" + this.props.orderId} className="form-control" placeholder="Username" aria-label="Username" aria-describedby="basic-addon1" ref={"deliveryDate" + this.props.orderId}/>
                            </div>
                        </div>                 
                    </div>
